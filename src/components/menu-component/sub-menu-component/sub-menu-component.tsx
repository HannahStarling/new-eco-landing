...
export const SubMenuComponent: FC<IProps> = ({ className, menu, handleScroll }) => {
  if (!menu) return null;
  return (
    <UiList type={ListType.vertical} className={classNames('submenu submenu__list', className)}>
      {Object.values(menu).map((menuItem, index) => (
        <UiListItem key={menuItem.name + index} className={'submenu__subitem subitem'}>
          {menuItem?.sectionID ? (
